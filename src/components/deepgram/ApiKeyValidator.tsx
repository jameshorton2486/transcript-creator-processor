...
export const ApiKeyValidator: React.FC<ApiKeyValidatorProps> = ({
  ...
  if (showAlert) {
    return (
      <Alert variant="destructive" className="py-2" role="alert" id={errorId}>
  ...
  if (keyStatus === "valid" && !showAlert) {
    return (
      <Alert variant="default" className="bg-green-50 text-green-800 border-green-200 py-2" role="status">
  ...
  if (keyFormatValid && !keyStatus && !showAlert) {
    return (
      <Alert variant="default" className="bg-amber-50 text-amber-800 border-amber-200 py-2" role="status">
        <AlertDescription>
