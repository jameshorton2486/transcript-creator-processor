...
/**
 * Transcribes an audio URL using Deepgram API
 */
export async function transcribeAudioUrl(
  ...
  try {
    ...
    // Create request payload with URL
    const payload = {
      url: url
    };

    // Perform request with proper Content-Type header for JSON payloads
    const response = await fetch(endpoint, {
      method: 'POST',
      headers: {
        'Authorization': `Token ${apiKey}`,
        'Content-Type': 'application/json', // Required for URL-based transcription
      },
