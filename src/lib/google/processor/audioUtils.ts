...
/**
 * Audio utilities adapter for AssemblyAI (was previously for Google STT)
...
/**
 * Convert audio to a format compatible with AssemblyAI
 */
export const convertAudioForAssemblyAI = async (audioData: ArrayBuffer): Promise<File> => {
  // Simply wrap the ArrayBuffer in a File object with appropriate type
  // AssemblyAI accepts various formats including WAV
  const file = new File([audioData], 'audio.wav', { type: 'audio/wav' });
  return file;
};

/**
 * Prepare audio data for transcription using AssemblyAI
 */
export const prepareAudioForTranscription = async (
  audioData: ArrayBuffer
): Promise<File> => {
  try {
    // For AssemblyAI, we can use the file directly without complex preprocessing
    return convertAudioForAssemblyAI(audioData);
  } catch (error) {
